...
//  Copyright (c) 2013-2015 SMipboard. All rights reserved.
//
...
extern const NSTimeInterval kSMAnimatedImageDelayTimeIntervalMinimum;

//
//  An `SMAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `SMAnimatedImageView`.
...
//
@interface SMAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `SMAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `SMAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
